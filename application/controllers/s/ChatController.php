...
class ChatController extends CI_Controller {
  ...
  public function index(){
    ...
    $data['strsubTitle']='';
    $list=[];
    if($_SESSION['myuser']['role'] == 'student'){
      $list = $this->UserModel->MentorList();
      $data['strTitle']='All Mentors';
      $data['strsubTitle']='Mentors';
      $data['chatTitle']='Select Mentor to Chat';
    }else{
      $list = $this->UserModel->StudentList();
      $data['strTitle']='All Students';
      $data['strsubTitle']='Students';
      $data['chatTitle']='Select Student to Chat';
    }
    $vendorslist=[];
    foreach($list as $u){
      $vendorslist[]=
      [
        'id' => $u['id'],
        'name' => $u['name'],
        'profile_pic' => $this->UserModel->PictureUrlById($u['id']),
      ];
    }
    $data['vendorslist']=$vendorslist;


    $this->load->view('account/user/chat',$data);
  }
  ...
  public function send_text_message(){
    ...
    $response='';
    if($query == true){
      $response = ['msg_status' => 1 ,'message' => '' ];
    }else{
      $response = ['msg_status' => 0 ,'message' => 'sorry we re having some technical problems. please try again !'];
    }
  ...
  public function get_chat_history_by_vendor(){
    ...
    $Logged_sender_id = $_SESSION['myuser']['id'];
    ...
    foreach($history as $chat):
      ...
      if($message=='NULL'){ //fetach media objects like images,pdf,documents etc
        ...
        $document_url = base_url('assets/uploads/attachments/'.$attachment_name);

        if($mime_type[0]=='image'){
          $messageBody.='<img src="'.$document_url.'" onClick="ViewAttachmentImage('."'".$document_url."'".','."'".$attachment_name."'".');" class="attachmentImgCls img-fluid">';
        }else{
          $messageBody='';
          $messageBody.='<div class="attachment">';
          $messageBody.='<h4 style="color: #fff;font-weight: bold;">Attachment:</h4>';
          $messageBody.='<p class="filename">';
          $messageBody.= $attachment_name;
          $messageBody.='</p>';

          $messageBody.='<div class="pull-'.$classBtn.'">';
          $messageBody.='<a download href="'.$document_url.'"><button type="button" id="'.$message_id.'" class="btn btn-success btn-sm btnFileOpen" style="color: #fff; font-weight: bold">Open</button></a>';
          $messageBody.='</div>';
          $messageBody.='</div>';
        }
